...
void BIOFFSRComponent::setup() { 
  ESP_LOGCONFIG(TAG, "Setting up BIOFFSR Component...");
...
void BIOFFSRComponent::dump_config() {
...
void BIOFFSRComponent::pid_update() {
  uint32_t now = millis();
  float tmp;
  ...
  if (!this->current_manual_override_){
	...
	if (this->current_output1_enable_ ){
      this->device_output1_->set_level(this->output_);
	}
	if (this->current_output2_enable_ ){
      this->device_output2_->set_level(this->output_);
	}
