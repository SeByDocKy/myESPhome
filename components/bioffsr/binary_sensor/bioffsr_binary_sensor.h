#pragma once

#include "esphome/components/binary_sensor/binary_sensor.h"
#include "../bioffsr.h"

namespace esphome {
namespace bioffsr {

class BIOFFSRBinarySensor : public Component, public Parented<BIOFFSRComponent> {
 public:
  void dump_config() override;
  void setup() override;
  
  void set_parent(BIOFFSRComponent *parent) { this->parent_ = parent; }
  void set_thermostat_cut_binary_sensor(binary_sensor::BinarySensor *thermostat_cut_binary_sensor) { this->thermostat_cut_binary_sensor_ = thermostat_cut_binary_sensor; }; 

 protected:
  binary_sensor::BinarySensor *thermostat_cut_binary_sensor_{nullptr};
  BIOFFSRComponent *parent_;
  void publish_data_();
};


} // bioffsr
} // esphome