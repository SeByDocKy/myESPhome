...
void JSY22X::on_modbus_data(const std::vector<uint8_t> &data) {
  ...
  if (this->read_data_ == 1){
    ...
    ESP_LOGD(TAG, "modbus address=%d, baudrate=%d, V=%.1f V, I=%.3f A, P=%.1f W, Q=%.1f VAr, S=%.1f VA, PF=%.2f, F=%.1f Hz, EP=%.1f kWh, EQ=%.1f kVArh, ACDC mode=%1.0f, P_dir=%1.0f, Q_dir=%1.0f, E+=%.1f, E-=%.1f , Q+=%.1f, Q-=%.1f", int(this->address_), int(this->parent_->this->parent_->get_baud_rate()) , voltage, current, active_power,
