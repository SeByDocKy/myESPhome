...
void OutputMaxNumber::setup() {
  ...
  if (!this->pref_.load(&value)) value = this->parent_->get_output_max()*100.0f;  // should be in [0-1]
  this->parent_->set_output_max(value);
  this->publish_state(value*100.0f);	


  // if (!this->pref_.load(&value)) value = this->parent_->get_output_max();  // should be in [0-1]
