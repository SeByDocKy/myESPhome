...
static const uint8_t JSY193_CMD_START_REGISTER = 0x100;
static const uint8_t JSY193_CMD_RESET_ENERGY = 0x42;
static const uint8_t JSY193_REGISTER_COUNT = 20;  // 10x 16-bit registers

void JSY193::on_modbus_data(const std::vector<uint8_t> &data) {
  if (data.size() < JSY193_REGISTER_COUNT*2) {
    ESP_LOGW(TAG, "Invalid size for JSY193!");
    return;
  }
  ...
  uint16_t raw_power_factor1 = jsy193_get_16bit(16);
  ...
  float current2 = ((1 - raw_sign)*raw_current - raw_sign*raw_current)/100.0f;  // max 4294967.295 A
  
  uint16_t raw_power   = jsy193_get_16bit(24);
  float power2 = (1 - raw_sign)*raw_power - raw_sign*raw_power;  // max 4294967.295 A
    

  float pos_energy2 = static_cast<float>(jsy193_get_32bit(28));
  float neg_energy2 = static_cast<float>(jsy193_get_32bit(32));
...
void PZEMAC::update() { this->send(JSY193_CMD_READ_IN_REGISTERS, JSY193_CMD_START_REGISTER , JSY193_REGISTER_COUNT); }
