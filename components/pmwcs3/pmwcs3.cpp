...
*/	
  ...
  if (this->ec_sensor_ != nullptr) {
	  ...
	  this->ec_sensor_->publish_state(ec);
	  ESP_LOGD(TAG, "ec: data[0]=%d, data[1]=%d, result=%f", data[0] , data[1] , ec);
  }
  ...
  if (this->temperature_sensor_ != nullptr) {
	  ...
	  this->temperature_sensor_->publish_state(temperature);
	  ESP_LOGD(TAG, "temp: data[0]=%d, data[1]=%d, result=%f", data[0] , data[1] , temperature); 
  }
  ...
  if (this->vwc_sensor_ != nullptr) {
	  ...
	  this->vwc_sensor_->publish_state(vwc);
	  ESP_LOGD(TAG, "vwc: data[0]=%d, data[1]=%d, result=%f", data[0] , data[1] , vwc);
  }
 // */
}
