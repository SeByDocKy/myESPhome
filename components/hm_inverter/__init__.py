...
CONFIG_SCHEMA = (
    cv.Schema({
        ...
        cv.Required(CONF_PINS): cv.Schema({
            ...
            cv.Required(CONF_EN): pins.internal_gpio_input_pin_schema,
            cv.Required(CONF_IRQ): pins.internal_gpio_input_pin_schema,
        }),
...
async def to_code(config):
    ...
    cg.add(var.set_mosi(await cg.gpio_pin_expression(config[CONF_PINS][CONF_MOSI])))
    cg.add(var.set_miso(await cg.gpio_pin_expression(config[CONF_PINS][CONF_MISO])))
