...
ActivationSwitch = dualpidpcm_ns.class_("ActivationSwitch", switch.Switch, cg.Component)
...
async def to_code(config):
    ...
    if activation_config := config.get(CONF_ACTIVATION):
        ...
        cg.add(dualpidpcm_component.set_activation_switch(s))
    ...
    if manual_override_config := config.get(CONF_MANUAL_OVERRIDE):
        ...
        await cg.register_parented(s, dualpidpcm_component)
        cg.add(dualpidpcm_component.set_manual_override_switch(s))
    ...
    if pid_mode_config := config.get(CONF_PID_MODE):
        ...
        await cg.register_parented(s, dualpidpcm_component)
        cg.add(dualpidpcm_component.set_pid_mode_switch(s)) 
