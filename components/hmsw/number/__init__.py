...
# Named "persistent", not just "power_limit_percent": unlike hm:/hms:, no
...
# (persistent_power_limit_percent) -- same command, same name, one is a
# number: and the other a float output:.
CONF_PERSISTENT_POWER_LIMIT_PERCENT = "persistent_power_limit_percent"

DEPENDENCIES = ["hmsw"]

HMSWPersistentPowerLimitPercentNumber = hmsw_ns.class_(
    "HMSWPersistentPowerLimitPercentNumber", number.Number
)

CONFIG_SCHEMA = cv.Schema(
    {
        cv.GenerateID(CONF_HMSW_ID): cv.use_id(HMSWComponent),
        cv.Optional(CONF_PERSISTENT_POWER_LIMIT_PERCENT): number.number_schema(
            HMSWPersistentPowerLimitPercentNumber,
...
async def to_code(config):
    ...
    if CONF_PERSISTENT_POWER_LIMIT_PERCENT in config:
        conf = config[CONF_PERSISTENT_POWER_LIMIT_PERCENT]
