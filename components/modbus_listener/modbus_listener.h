...
class ModbusListenerHub : public Component, public uart::UARTDevice {
 public:
  ...
  void set_add_crc(bool add_crc) { add_crc_ = add_crc; }
  void set_payload_length(uint8_t length) { payload_length_ = length; has_payload_length_ = true; }
 ...
 protected:
  ...
  bool has_payload_length_{false};
  ...
  std::vector<uint8_t> process_frame_data(const std::vector<uint8_t> &frame);
