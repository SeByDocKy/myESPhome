...
class VictronComponent : public uart::UARTDevice, public Component {
 public:
  ...
  void set_dc_input_current_sensor(sensor::Sensor *dc_input_current_sensor) {
    dc_input_current_sensor_ = dc_input_current_sensor;
  }
 ...
 protected:
  void handle_value_();
  static const std::unordered_map<std::string, std::function<void(VictronComponent &, const std::string &)>>
      &get_handlers_();
