...
static const char *const TAG = "hms_regulation";
static const float coeffP = 0.001f;
static const float coeffI = 0.0001f;
static const float coeffD = 0.001f;


void HMS_REGULATIONComponent::setup() { 
  ESP_LOGCONFIG(TAG, "Setting up HMS_REGULATIONComponent...");
...
void HMS_REGULATIONComponent::dump_config() {
...
void HMS_REGULATIONComponent::pid_update() {
  uint32_t now = millis();
  float tmp;
