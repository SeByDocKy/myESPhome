...
ModbusServerRegisterSchema = cv.Schema(
    {
        ...
        #cv.Required(CONF_UNIT_ID): cv.positive_int,
...
ModbusItemBaseSchema = cv.Schema(
    {
        ...
        #cv.Optional(CONF_UNIT_ID): cv.positive_int,
...
def validate_modbus_register(config):
    #if CONF_CUSTOM_COMMAND not in config and CONF_UNIT_ID not in config:
    if CONF_CUSTOM_COMMAND not in config and CONF_ADDRESS not in config:
        raise cv.Invalid(
            ...
            #f" {CONF_UNIT_ID} is a required property if '{CONF_CUSTOM_COMMAND}:' isn't used"
        )
...
def modbus_calc_properties(config):
    ...
    if CONF_CUSTOM_COMMAND in config:
        if CONF_ADDRESS not in config:
            ...
            #config[CONF_UNIT_ID] = binascii.crc_hqx(value, 0)
...
async def to_code(config):
    ...
    for conf in config.get(CONF_ON_COMMAND_SENT, []):
        ...
        await automation.build_automation(
            ...
            #trigger, [(cg.int_, "function_code"), (cg.int_, "unit_id")], conf
        )
...
async def register_modbus_device(var, config):
    ...
    #cg.add(var.set_address(config[CONF_UNIT_ID]))
